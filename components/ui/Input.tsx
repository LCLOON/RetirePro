...
// Percentage Select - dropdown for percentage values (iOS-friendly)
interface PercentSelectProps {
  label?: string;
  value: number;
  onChange: (value: number) => void;
  asDecimal?: boolean; // if true, value is stored as decimal (0.07), displayed as percent (7)
  min?: number; // minimum percent value (as displayed, e.g., 0)
  max?: number; // maximum percent value (as displayed, e.g., 15)
  step?: number; // step between values (as displayed, e.g., 0.5)
  hint?: string;
}
...
export function PercentSelect({ 
  ...
  for (let i = min; i <= max; i += step) {
    const displayPercent = i.toFixed(step < 1 ? 1 : 0);
    options.push({
  ...
  const handleChange = (val: string) => {
    ...
    if (!isNaN(numVal)) {
      onChange(asDecimal ? numVal / 100 : numVal);
    }
  };
    ...
    <div className="w-full">
      ...
      <div className="relative">
        <select
          ...
          title={label || 'Select percentage'}
          aria-label={label || 'Select percentage'}
          className="w-full px-4 py-2.5 rounded-lg border transition-colors duration-200
            appearance-none bg-slate-800 text-white cursor-pointer
            border-slate-600 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/30"
        >
